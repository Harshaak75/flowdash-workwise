...
router.post("/create", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), upload.single("file"), async (req, res) => {
    try {
        ...
        const tenantId = req.user.tenantId; // 🔑 SOURCE OF TRUTH
...
router.patch("/:taskId/status", auth_js_1.auth, async (req, res) => {
    try {
        ...
        if (!Object.values(client_1.TaskStatus).includes(newStatus)) {
...
router.post("/:id/transfer", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER", "PROJECT_MANAGER"), async (req, res) => {
    try {
        ...
        const assigneeUser = await db_1.default.user.findFirst({
            where: {
                id: assigneeId,
                tenantId,
            },
...
// manager
router.get("/dashboard/manager", auth_js_1.auth, (0, role_js_1.requireRole)("MANAGER"), async (req, res) => {
    try {
...
// project manager
router.get("/dashboard/project-manager", auth_js_1.auth, (0, role_js_1.requireRole)("PROJECT_MANAGER"), async (req, res) => {
    const { tenantId } = req.user;
    /* ---------------- Employees ---------------- */
    ...
    const completedTasks = tasks.filter(t => t.status === "DONE").length;
    const inProgressTasks = tasks.filter(t => t.status === "WORKING").length;
    const totalAssignedHours = tasks.reduce((sum, t) => sum + (t.assignedHours || 0), 0);
    res.json({
        cards: {
            totalEmployees,
            activeEmployees,
